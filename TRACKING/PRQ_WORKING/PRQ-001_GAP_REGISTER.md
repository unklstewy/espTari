...
| PRQ1-GAP-003 | save/restore | Suspend/restore guard and negative matrix rows validated (`paused` mode, not-suspended, bad-request, not-found, incompatible). Residual is full schema/ABI/profile compatibility validator implementation depth. | High | Engineering | Implement real compatibility validator path (`RCOMP-*`, `RCOMP-VAL-*`) and replace synthetic incompatible trigger with validator-backed classification evidence. | PRQ1-GAP-001 | 2026-03-06 | Partially Closed |
