...
- Snapshot schema v1 + component state block contract (`snapshot_schema_v1`, `snapshot_component_state_blocks_v1`, required ST baseline component fields, scheduler/media binding restore fields, and compatibility metadata) is defined in `docs/EMU_ENGINE_V2_API_SPEC.md` section `11.6` and is the canonical source.
- Snapshot component serializer/deserializer contract (`snapshot_component_serializers_v1`, checks `SER-ORD-01`, `SER-ORD-02`, `SER-VAL-01`, and deterministic serializer fingerprinting) is defined in `docs/EMU_ENGINE_V2_API_SPEC.md` section `11.6` and is the canonical source.
- Snapshot persistence atomic backend contract (`snapshot_persistence_atomic_v1`, staged temp writes + atomic rename commit, save-pointer commit ordering, checks `PERSIST-ATOMIC-01/02` and `PERSIST-WRITE-01/02`) is defined in `docs/EMU_ENGINE_V2_API_SPEC.md` section `11.6` and is the canonical source.
- Snapshot persistence backend stores metadata/index files under SD-card state path `/sdcard/saves/states/` (not SPIFFS), consistent with the canonical storage authority model.
- `POST /api/v2/engine/session/suspend-save` now emits deterministic transition diagnostics (`lifecycle_transition`, ordered `transition_events`) and returns rollback diagnostics on persistence failure with `rollback_to_state` / `rollback_state_preserved` semantics.
...
- `POST /api/v2/engine/state/save` now runs deterministic serializer/deserializer routines for ST baseline component blocks (`cpu`, `glue_mmu_shifter`, `mfp`, `acia_ikbd`, `dma_fdc`, `psg`) and projects serializer diagnostics via `serializer_checks` + `serializer_fingerprint`.
...
- `POST /api/v2/media/rom/attach` enforces deterministic request validation and ROM catalog-binding checks (`BAD_REQUEST`, `CATALOG_ENTRY_NOT_FOUND`, `CONFLICT`, `INVALID_SESSION_STATE`) before attach projection.
...
- `POST /api/v2/debug/clock/mode` enforces deterministic bounds guards for realtime/slow-motion/single_step mode+ratio pairings and returns structured `DEBUG_CLOCK_INVALID` diagnostics with bound guard IDs (`CLOCK-BOUND-01/02`).
...
- Debug clock runtime now projects arbitration hook-layer metadata (`arb_pre_tick -> arb_component_step -> arb_post_tick`) and deterministic timestamp emitter sequencing (`emit_seq`) in clock state telemetry.
