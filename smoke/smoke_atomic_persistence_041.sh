...
OUT="captures/atomic_persistence_041_smoke_postfix_${TS}.txt"
mkdir -p captures
: > "$OUT"
LAST_BODY=""
...
call_json "save_atomic_baseline" POST "/api/v2/engine/state/save" '{"session_id":"ses_local","name":"atomic_041_baseline"}' "^200$"
BASELINE_SNAPSHOT_ID="$(extract_json 'd["data"]["snapshot_id"]')"

call_json "list_before_interrupt" GET "/api/v2/engine/state/list?session_id=ses_local" "" "^200$"
COUNT_BEFORE="$(extract_json 'int(d["data"]["total_entries"])')"

call_json "save_force_interrupt" POST "/api/v2/engine/state/save?force_persist_interrupt=1" '{"session_id":"ses_local","name":"atomic_041_interrupted"}' "^500$"
CHECK_ID="$(extract_json 'd["error"]["details"]["check_id"]')"
if [[ "$CHECK_ID" != "PERSIST-ATOMIC-01" && "$CHECK_ID" != "PERSIST-ATOMIC-02" ]]; then
  echo "atomic_check_id=failed got=$CHECK_ID" | tee -a "$OUT"
  exit 1
fi

echo "atomic_check_id=pass check_id=$CHECK_ID" | tee -a "$OUT"

call_json "list_after_interrupt" GET "/api/v2/engine/state/list?session_id=ses_local" "" "^200$"
COUNT_AFTER="$(extract_json 'int(d["data"]["total_entries"])')"
if [[ "$COUNT_AFTER" != "$COUNT_BEFORE" ]]; then
  echo "atomic_commit_guard=failed before=$COUNT_BEFORE after=$COUNT_AFTER" | tee -a "$OUT"
  exit 1
fi

echo "atomic_commit_guard=pass before=$COUNT_BEFORE after=$COUNT_AFTER baseline_snapshot_id=$BASELINE_SNAPSHOT_ID" | tee -a "$OUT"
