...
OUT="$ROOT_DIR/captures/ebin_207_dma_fdc_shared_build_${TS}.txt"
BUILD_SCRIPT="$ROOT_DIR/tools/ebin_builder/build_st_storage_dma_fdc_shared.sh"
...
STORAGE_DIR="$TARGET/ebins/atari_st/storage"
EBIN_PATH="$STORAGE_DIR/st.storage.dma_fdc-1.0.0.ebin"
...
assert index["component"] == "storage"
assert index["module_id"] == "st.storage.dma_fdc"
assert index["winner"] == "1.0.0"
assert index["versions"] == ["1.0.0"]
assert index["files"] == ["st.storage.dma_fdc-1.0.0.ebin"]
...
assert manifest["component"] == "storage"
assert manifest["module_id"] == "st.storage.dma_fdc"
assert manifest["winner"] == "st.storage.dma_fdc@1.0.0"
assert manifest["index"] == "index.json"
assert manifest["layout_version"] == "st520_st1040_layout_v1"

print("dma_fdc_ebin_header=pass")
print("dma_fdc_metadata_contract=pass")
PY
