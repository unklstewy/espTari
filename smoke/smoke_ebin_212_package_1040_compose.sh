...
OUT="$ROOT_DIR/captures/ebin_212_package_1040_compose_${TS}.txt"
...
PACKAGE_DIR="$BASE_DIR/packages/st.profile.1040"
...
for path in \
  ...
  "$BASE_DIR/machine_profile/st.profile.1040-1.0.0.ebin" \
  "$PACKAGE_INDEX" \
  "$PACKAGE_MANIFEST"; do
  if [[ ! -f "$path" ]]; then
...
assert index["profile_module"] == "st.profile.1040"
assert index["winner"] == "st.profile.1040@1.0.0"
...
assert components["machine_profile"] == "st.profile.1040@1.0.0"

assert manifest["schema"] == "st_machine_package_manifest_v1"
assert manifest["machine"] == "atari_st"
assert manifest["profile_module"] == "st.profile.1040"
...
assert placement["package_rel"] == "packages/st.profile.1040"
...
print("package_1040_metadata_contract=pass")
print("package_1040_placement_contract=pass path=ebins/atari_st/packages/st.profile.1040")
PY
