...
OUT="$ROOT_DIR/captures/ebin_208_audio_gpio_psg_shared_build_${TS}.txt"
BUILD_SCRIPT="$ROOT_DIR/tools/ebin_builder/build_st_audio_gpio_psg_shared.sh"
...
AUDIO_DIR="$TARGET/ebins/atari_st/audio_gpio"
EBIN_PATH="$AUDIO_DIR/st.audio_gpio.psg-1.0.0.ebin"
...
assert component_type == 3, f"component type must be AUDIO(3), got {component_type}"
...
assert index["component"] == "audio_gpio"
assert index["module_id"] == "st.audio_gpio.psg"
assert index["winner"] == "1.0.0"
assert index["versions"] == ["1.0.0"]
assert index["files"] == ["st.audio_gpio.psg-1.0.0.ebin"]
...
assert manifest["component"] == "audio_gpio"
assert manifest["module_id"] == "st.audio_gpio.psg"
assert manifest["winner"] == "st.audio_gpio.psg@1.0.0"
assert manifest["index"] == "index.json"
assert manifest["layout_version"] == "st520_st1040_layout_v1"

print("audio_gpio_psg_ebin_header=pass")
print("audio_gpio_psg_metadata_contract=pass")
PY
