...
OUT="$ROOT_DIR/captures/ebin_205_mfp_shared_build_${TS}.txt"
BUILD_SCRIPT="$ROOT_DIR/tools/ebin_builder/build_st_chipset_mfp_shared.sh"
...
EBIN_PATH="$CHIPSET_DIR/st.chipset.mfp-1.0.0.ebin"
...
assert index["module_id"] == "st.chipset.mfp"
assert index["winner"] == "1.0.0"
assert index["versions"] == ["1.0.0"]
assert index["files"] == ["st.chipset.mfp-1.0.0.ebin"]
...
assert manifest["module_id"] == "st.chipset.mfp"
assert manifest["winner"] == "st.chipset.mfp@1.0.0"
assert manifest["index"] == "index.json"
assert manifest["layout_version"] == "st520_st1040_layout_v1"

print("mfp_ebin_header=pass")
print("mfp_metadata_contract=pass")
PY
