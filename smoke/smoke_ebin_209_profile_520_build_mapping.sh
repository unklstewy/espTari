...
OUT="$ROOT_DIR/captures/ebin_209_profile_520_build_mapping_${TS}.txt"
BUILD_SCRIPT="$ROOT_DIR/tools/ebin_builder/build_st_profile_520_machine_profile.sh"
...
EBIN_PATH="$PROFILE_DIR/st.profile.520-1.0.0.ebin"
...
assert index["module_id"] == "st.profile.520"
assert index["winner"] == "1.0.0"
assert index["versions"] == ["1.0.0"]
assert index["files"] == ["st.profile.520-1.0.0.ebin"]
...
assert manifest["module_id"] == "st.profile.520"
assert manifest["winner"] == "st.profile.520@1.0.0"
assert manifest["resolved_profile"] == "st_520_pal"
...
print("profile_520_ebin_header=pass")
print("profile_520_metadata_contract=pass")
print("runtime_mapping_proof=pass module=st.profile.520 profile=st_520_pal")
PY
