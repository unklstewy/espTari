...
OUT="captures/ebin_s10_016_release_reviewpack_integration_${TS}.txt"
mkdir -p captures
: > "$OUT"
LAST_BODY=""
...
for run in 1 2 3; do
  ...
  review_state="$(extract_json 'd["data"]["state"]')"
  review_uri="$(extract_json 'd["data"]["review_pack_uri"]')"

  if [[ "$package_state" != "ready" || "$review_state" != "ready" ]]; then
    echo "release_reviewpack_contract=failed run=${run} package=${package_state} review=${review_state}" | tee -a "$OUT"
    exit 1
  fi

  review_path_ok="$(/home/sannis/electronics/esp32p4/projects/espTari/.venv/bin/python -c 'import sys
uri=sys.argv[1]
print(str("/review_pack_" in uri and uri.endswith(".json")))' "$review_uri")"
  if [[ "$review_path_ok" != "True" ]]; then
    echo "release_reviewpack_path=failed run=${run} uri=${review_uri}" | tee -a "$OUT"
    exit 1
  fi

  run_signature="manifest=${checks_total}:${checks_enabled};package=${package_state};review=${review_state}:json"
