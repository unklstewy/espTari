...
OUT="$ROOT_DIR/captures/ebin_206_acia_ikbd_shared_build_${TS}.txt"
BUILD_SCRIPT="$ROOT_DIR/tools/ebin_builder/build_st_io_acia_ikbd_shared.sh"
...
IO_DIR="$TARGET/ebins/atari_st/io"
EBIN_PATH="$IO_DIR/st.io.acia_ikbd-1.0.0.ebin"
INDEX_PATH="$IO_DIR/index.json"
MANIFEST_PATH="$IO_DIR/manifest.json"
...
assert index["component"] == "io"
assert index["module_id"] == "st.io.acia_ikbd"
assert index["winner"] == "1.0.0"
assert index["versions"] == ["1.0.0"]
assert index["files"] == ["st.io.acia_ikbd-1.0.0.ebin"]
...
assert manifest["component"] == "io"
assert manifest["module_id"] == "st.io.acia_ikbd"
assert manifest["winner"] == "st.io.acia_ikbd@1.0.0"
assert manifest["index"] == "index.json"
assert manifest["layout_version"] == "st520_st1040_layout_v1"

print("acia_ikbd_ebin_header=pass")
print("acia_ikbd_metadata_contract=pass")
PY
