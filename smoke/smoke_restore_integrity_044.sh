...
OUT="captures/restore_integrity_044_smoke_postfix_${TS}.txt"
mkdir -p captures
: > "$OUT"
LAST_BODY=""
...
call_json "status_before" GET "/api/v2/engine/status" "" "^200$"
STATE_BEFORE="$(extract_json 'd["data"]["session_state"]')"

call_json "save_snapshot" POST "/api/v2/engine/state/save" '{"session_id":"ses_local","name":"restore_integrity_044"}' "^200$"
SNAPSHOT_ID="$(extract_json 'd["data"]["snapshot_id"]')"

call_json "restore_success" POST "/api/v2/engine/state/restore" "{\"session_id\":\"ses_local\",\"snapshot_id\":\"${SNAPSHOT_ID}\"}" "^200$"

call_json "restore_forced_integrity_fail" POST "/api/v2/engine/state/restore?force_integrity_fail=1" "{\"session_id\":\"ses_local\",\"snapshot_id\":\"${SNAPSHOT_ID}\"}" "^409$"

call_json "status_after_fail" GET "/api/v2/engine/status" "" "^200$"
STATE_AFTER="$(extract_json 'd["data"]["session_state"]')"
if [[ "$STATE_BEFORE" != "$STATE_AFTER" ]]; then
  echo "State changed after failed restore: before=$STATE_BEFORE after=$STATE_AFTER"
  exit 1
fi

call_json "restore_missing" POST "/api/v2/engine/state/restore" '{"session_id":"ses_local","snapshot_id":"missing_044"}' "^404$"
