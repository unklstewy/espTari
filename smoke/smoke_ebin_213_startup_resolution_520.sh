...
OUT="$ROOT_DIR/captures/ebin_213_startup_resolution_520_${TS}.txt"
...
PACKAGE_INDEX="$TARGET/ebins/atari_st/packages/st.profile.520/index.json"

touch "$PROFILE_DIR/st.profile.520-0.9.0.ebin"
touch "$PROFILE_DIR/st.profile.520-1.2.0.ebin"

for path in \
  "$PROFILE_DIR/st.profile.520-0.9.0.ebin" \
...
assert package_index["profile_module"] == "st.profile.520"
assert package_index["winner"] == "st.profile.520@1.0.0"

loader_text = Path(loader_path).read_text(encoding="utf-8")
assert 'strcmp(module_id, "st.profile.520") == 0' in loader_text
assert 'return "st_520_pal";' in loader_text
...
assert selected_module == "st.profile.520"
assert selected_version == "1.2.0"
assert selected_profile == "st_520_pal"

print("startup_resolution_determinism=pass")
print("startup_resolution_winner=st.profile.520@1.2.0")
print("startup_resolution_profile=st_520_pal")
PY
