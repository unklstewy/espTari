...
OUT="captures/ebin_s10_015_release_checklist_integration_${TS}.txt"
mkdir -p captures
: > "$OUT"
LAST_BODY=""
...
for run in 1 2 3; do
  ...
  status_state="$(extract_json 'd["data"]["state"]')"
  completed_cases="$(extract_json 'int(d["data"].get("progress", {}).get("completed_cases", 0))')"
  total_cases="$(extract_json 'int(d["data"].get("progress", {}).get("total_cases", 0))')"
  status_total="$(extract_json 'int(d["data"].get("summary", {}).get("cases_total", 0))')"
  status_passed="$(extract_json 'int(d["data"].get("summary", {}).get("cases_passed", 0))')"
  status_failed="$(extract_json 'int(d["data"].get("summary", {}).get("cases_failed", 0))')"

  if [[ "$collection_state" != "completed" || "$package_state" != "ready" || "$status_state" != "completed" || "$status_failed" != "0" ]]; then
    echo "release_checklist_contract=failed run=${run} collection=${collection_state} package=${package_state} status=${status_state}:${status_total}:${status_passed}:${status_failed}:${completed_cases}/${total_cases}" | tee -a "$OUT"
    exit 1
  fi

  run_signature="manifest=${checks_total}:${checks_enabled};package=${package_state};checklist=${status_state}:${status_total}:${status_passed}:${status_failed};progress=${completed_cases}/${total_cases}"
