...
OUT="captures/t110_t111_section11_runtime_${TS}.txt"
mkdir -p captures
: > "$OUT"
LAST_BODY=""
...
for run in 1 2 3; do
  ...
  manifest_id="mf_t110_111_run_${run}"
  manifest_payload="{\"manifest_id\":\"${manifest_id}\",\"manifest\":{\"manifest_version\":1,\"target_machine\":\"atari_st\",\"profile\":\"st_520_pal\",\"suite\":\"t110_111\",\"cases\":[{\"case_id\":\"mfp_irq_contract\",\"kind\":\"api\",\"target\":\"/api/v2/conformance/subsystems/suites/report\",\"assertions\":[\"status_200\"]}]}}"
  ...
  scaffold_state="$(extract_json 'd["data"]["state"]')"
  ...
  fixture_schema_version="$(extract_json 'd["data"]["schema_version"]')"
  ...
  suite_total="$(extract_json 'int(d["data"]["cases_total"])')"
  suite_passed="$(extract_json 'int(d["data"]["cases_passed"])')"
  suite_failed="$(extract_json 'int(d["data"]["cases_failed"])')"

  call_json "subsystem_suite_report_${run}" GET "/api/v2/conformance/subsystems/suites/report?suite_run_id=${suite_run_id}" "" "^200$"
  report_total="$(extract_json 'int(d["data"]["summary"]["cases_total"])')"
  report_passed="$(extract_json 'int(d["data"]["summary"]["cases_passed"])')"
  report_failed="$(extract_json 'int(d["data"]["summary"]["cases_failed"])')"

  if [[ "$scaffold_state" != "ready" || "$suite_state" != "completed" || "$suite_failed" != "0" || "$report_failed" != "0" ]]; then
    echo "section11_contract=failed run=${run} scaffold=${scaffold_state} suite=${suite_state}:${suite_total}:${suite_passed}:${suite_failed} report=${report_total}:${report_passed}:${report_failed}" | tee -a "$OUT"
    exit 1
  fi

  run_signature="scaffold=${scaffold_state};fixture_schema=${fixture_schema_version};suite=${suite_state}:${suite_total}:${suite_passed}:${suite_failed};report=${report_total}:${report_passed}:${report_failed}"
