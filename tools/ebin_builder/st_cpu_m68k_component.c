...
static int cpu_execute(int cycles)
{
    ...
    s_cpu.steps_total += (uint32_t)cycles;
    s_cpu.state.pc += (uint32_t)(cycles * 2);
    return cycles;
}

static void cpu_set_irq(uint8_t level)
{
    s_cpu.state.sr = (uint16_t)((s_cpu.state.sr & 0xF8FFu) | ((uint16_t)(level & 0x07u) << 8));
}
