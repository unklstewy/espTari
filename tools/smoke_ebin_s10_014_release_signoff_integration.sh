...
OUT="captures/ebin_s10_014_release_signoff_integration_${TS}.txt"
mkdir -p captures
: > "$OUT"
LAST_BODY=""
...
for run in 1 2 3; do
  ...
  report_sha="$(extract_json 'd["data"]["sha256"]')"
  ...
  call_json "checklist_status_${run}" GET "/api/v2/conformance/harness/checklist/run/status?runner_id=${runner_id}" "" "^200$"
  status_state="$(extract_json 'd["data"].get("state", "unknown")')"
  status_total="$(extract_json 'int(d["data"].get("summary", {}).get("cases_total", 0))')"
  status_failed="$(extract_json 'int(d["data"].get("summary", {}).get("cases_failed", 0))')"
  ...
  review_state="$(extract_json 'd["data"].get("state", "ready")')"
  review_pack_id="$(extract_json 'd["data"]["review_pack_id"]')"

  signoff_payload="{\"review_pack_id\":\"${review_pack_id}\",\"signoff\":{\"requested_by\":\"qa_lead\",\"approver\":\"product_owner\",\"label\":\"s10_014_release_gate\"}}"
  call_json "signoff_bundle_assemble_${run}" POST "/api/v2/conformance/harness/signoff-bundle/assemble" "$signoff_payload" "^200$"
  signoff_state="$(extract_json 'd["data"].get("state", "assembled")')"
  signoff_label="$(extract_json 'd["data"].get("label", "s10_014_release_gate")')"

  if [[ "$collection_state" != "completed" || "$package_state" != "ready" || "$status_failed" != "0" ]]; then
    echo "release_contract=failed run=${run} collection=${collection_state} package=${package_state} status=${status_state} failed=${status_failed}" | tee -a "$OUT"
    exit 1
  fi

  run_signature="manifest=${checks_total}:${checks_enabled};report_sha=${report_sha};checklist=${status_state}:${status_total}:${status_failed};review=${review_state};signoff=${signoff_state}:${signoff_label}"
